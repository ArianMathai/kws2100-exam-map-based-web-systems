...
function EuropeAside() {
    ...
    const zoomToFeature = useCallback((feature: CountryFeature) => {
        ...
        if (!extent || extent.every((num) => num === Infinity || num === -Infinity)) {
    ...
    return (
        <>
            {!isAsideVisible ? <button className="zoom-btn" onClick={handleZoomEnd}>Go back!</button> :
            ...
            {clickedCountry ? (<div ref={olRef} className={"clickedCountryOl"}>
