...
function Routing() {
  ...
  const draw = useMemo(
    () => new Draw({ source, type: "LineString", maxPoints: 2 }),
    [source],
  );

  const [origin, setOrigin] = useState<Coordinate | undefined>(undefined);
  const [destination, setDestination] = useState<Coordinate | undefined>(
    undefined,
  );
  ...
  const transportOptions = [
    { value: "default", label: "Choose transport" },
  ...
  async function fetchRoute() {
    ...
    try {
      ...
      setErrorMessage(
        "Error fetching route data. Please choose mode of transportation.",
      );
    }
  }
  ...
  useEffect(() => {
    if (origin && destination && selectedOption !== "default") {
      setRequestUrl(
        `https://routing.openstreetmap.de/${selectedOption}/route/v1/routed-foot/${origin[0]},${origin[1]};${destination[0]},${destination[1]}?overview=false&alternatives=true&steps=true`,
      );
    }
      ...
      {distance && duration ? (
        <div className={"clickedFeature"}>
          <div className={"clickedFeatureBox"}>
            <button className="exit-btn" onClick={closeBox}>
              X
            </button>
            <p>Distance: {distance} km</p>
            {duration >= 60 ? (
              <p>
                Duration: {Math.floor(duration / 60)} hours {duration % 60} min
              </p>
            ) : (
